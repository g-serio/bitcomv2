...
END_OF_FILE_CONTENT
mkdir -p "src/components/b2b-solutions"
echo "Creating src/components/b2b-solutions/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/b2b-solutions/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/b2b-solutions/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/b2b-solutions/index.ts"
export { B2BSolutions } from './View';
export { B2BSchema } from './schema';
export type { B2BData, B2BSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/b2b-solutions/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/b2b-solutions/schema.ts"
import { z } from 'zod';
...
END_OF_FILE_CONTENT
echo "Creating src/components/b2b-solutions/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/b2b-solutions/types.ts"
import { z } from 'zod';
...
export { ContactSection } from './View';
export { ContactSchema } from './schema';
export type { ContactData, ContactSettings } from './types';
...
export const Footer: React.FC<{ data: FooterData; settings: FooterSettings }> = ({ data }) => {
  ...
  return (
    <footer 
      ...
      className="relative z-0 bg-[var(--local-bg)] border-t border-[var(--local-border)] pt-20 pb-10"
    >
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          <div className="md:col-span-5">
            <h3 className="font-display text-3xl font-semibold mb-6 italic" data-jp-field="brandName">{data.brandName}</h3>
            <p className="text-sm opacity-60 max-w-sm mb-8 leading-relaxed" data-jp-field="address">{data.address}</p>
            <div className="space-y-1">
              <p className="text-xs font-mono uppercase tracking-widest opacity-40">Telefono</p>
              <p className="font-display text-lg" data-jp-field="phone">{data.phone}</p>
            </div>
          </div>
          
          <div className="md:col-span-3">
            <h4 className="font-mono text-[10px] uppercase tracking-[0.2em] opacity-40 mb-6">Navigazione</h4>
            <ul className="space-y-4">
              {navItems.map((link, i) => (
                <li key={i}>
                  <a href={link.href} className="font-display text-lg hover:text-[var(--local-primary)] transition-colors">
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          <div className="md:col-span-4 flex flex-col justify-between">
            <div>
              <h4 className="font-mono text-[10px] uppercase tracking-[0.2em] opacity-40 mb-6">Dati Societari</h4>
              <p className="text-xs leading-relaxed opacity-60">
                <span className="block mb-1" data-jp-field="legalName">{data.legalName}</span>
                <span data-jp-field="piva">P.IVA: {data.piva}</span>
              </p>
            </div>
          </div>
        </div>

        <Separator className="my-12 opacity-10 bg-[var(--local-text)]" />
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-mono uppercase tracking-widest opacity-40">
          <span data-jp-field="copyright">{data.copyright}</span>
          <div className="flex gap-6">
            <span>Made in Palermo</span>
...
export const FooterSchema = BaseSectionData.extend({
  brandName: z.string().describe('ui:text'),
  legalName: z.string().describe('ui:text'),
  piva: z.string().describe('ui:text'),
  address: z.string().describe('ui:textarea'),
  phone: z.string().describe('ui:text'),
  copyright: z.string().describe('ui:text'),
  menu: z.array(FooterLink).optional().describe('ui:list')
});
...
export const Header: React.FC<{ data: HeaderData; settings: HeaderSettings }> = ({ data }) => {
  ...
  return (
    <header
      ...
      className="sticky top-0 z-50 w-full border-b border-[var(--local-border)] bg-[var(--local-bg)]/80 backdrop-blur-md"
    >
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
        {data.announcement && (
          <div className="hidden py-2 text-center text-[10px] font-mono uppercase tracking-[0.2em] opacity-60 md:block" data-jp-field="announcement">
            {data.announcement}
          </div>
        )}
        <div className="flex h-20 items-center justify-between">
          <a href="/" className="group flex items-baseline gap-1">
            <span className="font-display text-2xl font-semibold tracking-tight text-[var(--local-text)]" data-jp-field="logoText">
              {data.logoText}
            </span>
            {data.logoHighlight && (
              <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--local-primary)]" data-jp-field="logoHighlight">
                {data.logoHighlight}
              </span>
            )}
          </a>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button className="bg-[var(--local-primary)] text-white rounded-none font-display italic px-6">
                Contatti
              </Button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 lg:hidden">
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[var(--local-bg)] border-l-[var(--local-border)]">
                <SheetHeader>
                  <SheetTitle className="font-display italic text-left">Menu</SheetTitle>
                </SheetHeader>
                <nav className="mt-8 flex flex-col gap-6">
                  {navItems.map((item, i) => (
                    <a key={i} href={item.href} className="text-lg font-display border-b border-[var(--local-border)] pb-2">
                      {item.label}
                    </a>
                  ))}
                </nav>
...
END_OF_FILE_CONTENT
mkdir -p "src/components/history-strip"
echo "Creating src/components/history-strip/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-strip/View.tsx"
import React from 'react';
...
END_OF_FILE_CONTENT
echo "Creating src/components/history-strip/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-strip/index.ts"
export { HistoryStrip } from './View';
export { HistorySchema } from './schema';
export type { HistoryData, HistorySettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/history-strip/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-strip/schema.ts"
import { z } from 'zod';
import { BaseSectionData } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/history-strip/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-strip/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/services-bento"
echo "Creating src/components/services-bento/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/services-bento/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/services-bento/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/services-bento/index.ts"
export { ServicesBento } from './View';
export { ServicesBentoSchema } from './schema';
export type { ServicesBentoData, ServicesBentoSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/services-bento/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/services-bento/schema.ts"
import { z } from 'zod';
import { BaseSectionData, BaseArrayItem } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/services-bento/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/services-bento/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/testimonials-grid"
echo "Creating src/components/testimonials-grid/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/testimonials-grid/View.tsx"
import React from 'react';
...
END_OF_FILE_CONTENT
echo "Creating src/components/testimonials-grid/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/testimonials-grid/index.ts"
export { TestimonialsGrid } from './View';
export { TestimonialsSchema } from './schema';
export type { TestimonialsData, TestimonialsSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/testimonials-grid/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/testimonials-grid/schema.ts"
import { z } from 'zod';
import { BaseSectionData, BaseArrayItem } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/testimonials-grid/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/testimonials-grid/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
{
  "main": [
    { "label": "Soluzioni Privati", "href": "#servizi" },
    { "label": "Business & B2B", "href": "#aziende" },
...
cat << 'END_OF_FILE_CONTENT' > "src/data/config/site.json"
{
  "header": {
    ...
    "data": {
      ...
      "announcement": "Siamo in Via Simone Cuccia 1B, Palermo — Dal 1996",
  ...
  "footer": {
    ...
    "data": {
      "brandName": "BIT COM",
      "legalName": "B.S. Informatica di Borgese Marco & C. S.n.c.",
      "piva": "06244880826",
      "address": "Via Simone Cuccia 1B, 90144 Palermo (Zona Libertà)",
      "phone": "091 306740",
      "copyright": "© 2024 BIT COM. Tutti i diritti riservati.",
      "menu": { "$ref": "../config/menu.json#/footer" }
    },
    "settings": {}
  },
  "identity": { "title": "BIT COM | Informatica a Palermo" },
  "pages": []
}
...
cat << 'END_OF_FILE_CONTENT' > "src/data/pages/home.json"
{
  "id": "home",
  "slug": "home",
  "meta": { "title": "Home | BIT COM Palermo", "description": "Soluzioni informatiche per privati e aziende a Palermo dal 1996." },
  "sections": [
    {
      "id": "hero-1",
      "type": "hero",
      "data": {
        "label": "Trent'anni di informatica",
        "title": "Soluzioni Tecnologiche",
        "titleHighlight": "con Rigore ed Esperienza.",
        "description": "Dalla riparazione di smartphone alle reti aziendali complesse. A Palermo, siamo il punto di riferimento per chi cerca competenza senza compromessi.",
        "primaryCta": { "id": "cta-1", "label": "Servizi Aziendali", "href": "#aziende" },
        "secondaryCta": { "id": "cta-2", "label": "Assistenza Privati", "href": "#servizi" },
        "image": { "url": "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=2070&auto=format&fit=crop", "alt": "Workstation professionale" }
      }
    },
    {
      "id": "history-1",
      ...
      "data": {
        "year": "1996",
        "title": "Una storia che attraversa le epoche.",
        "body": "Abbiamo iniziato quando un computer in casa era un'eccezione. Oggi progettiamo il futuro digitale delle aziende di Palermo, mantenendo la stessa cortesia e onestà del primo giorno."
      }
    },
    {
      "id": "servizi-bento",
      "type": "services-bento",
      "data": {
        "label": "Assistenza & Hardware",
        "title": "Cosa facciamo per te.",
        "items": [
          { "id": "s1", "title": "Riparazione Mobile", "body": "Smartphone e tablet Apple e Android. Interventi rapidi e componenti garantite.", "icon": "Smartphone", "size": "large" },
          { "id": "s2", "title": "PC & Notebook", "body": "Vendita e assistenza. Nuovo e rigenerato certificato.", "icon": "Monitor", "size": "small" },
          { "id": "s3", "title": "Consumabili", "body": "Cartucce e toner compatibili per ogni marca.", "icon": "Printer", "size": "small" },
          { "id": "s4", "title": "Punto Linkem", "body": "Connettività internet veloce senza linea fissa.", "icon": "Wifi", "size": "small" }
        ]
      }
    },
    {
      ...
      "type": "b2b-solutions",
      "data": {
        "title": "Soluzioni Corporate.",
        "subtitle": "Progettiamo l'infrastruttura del tuo business, dalla rete alla formazione del personale.",
        "features": [
          { "id": "f1", "title": "Reti Aziendali", "desc": "Installazione e gestione di infrastrutture IT sicure e scalabili." },
          { "id": "f2", "title": "Sviluppo Software", "desc": "Soluzioni personalizzate per l'automazione e l'organizzazione." },
          { "id": "f3", "title": "Formazione IT", "desc": "Corsi su misura per ottimizzare i flussi di lavoro aziendali." }
        ]
      }
    },
    {
      ...
      "data": {
        "title": "Dicono di noi.",
        "items": [
          { "id": "t1", "quote": "Qualità e servizio eccellenti! Sicuramente consigliato.", "author": "Vito Luca" },
          { "id": "t2", "quote": "Molto preparati e professionali, cortesia e soluzioni ottimali li distinguono.", "author": "Domenico Scammacca" },
          { "id": "t3", "quote": "Professionalità e puntualità.", "author": "Antonino Matranga" }
        ]
      }
    },
    {
      ...
      "data": {
        ...
        "address": "Via Simone Cuccia 1B\n90144 Palermo (Zona Libertà)",
        "hours": "Lunedì–Venerdì: 09:00–13:00, 16:00–19:30\nSabato: 10:00–13:00\nDomenica: Chiuso",
        "phone": "091 306740",
        "email": "info@bitcom2009.it"
      }
    }
  ]
}
...
:root {
  ...
  --theme-radius-md: 0px;
  --theme-radius-sm: 0px;
  --theme-font-primary: "Source Serif 4", serif;
...
import { B2BSolutions } from '@/components/b2b-solutions';
...
const addableSectionTypes = [
  'hero', 'services-bento', 'b2b-solutions', 'history-strip', 'testimonials-grid', 'contact-section'
] as const;

const sectionTypeLabels: Record<string, string> = {
  'hero': 'Hero Principale',
  'services-bento': 'Griglia Servizi Privati',
  'b2b-solutions': 'Soluzioni Aziendali',
  'history-strip': 'Fascia Storica',
  'testimonials-grid': 'Recensioni Clienti',
  'contact-section': 'Contatti e Mappa',
};

function getDefaultSectionData(type: string): Record<string, unknown> {
  switch (type) {
    case 'hero': return { title: 'Nuovo Hero', description: 'Testo descrizione...' };
    case 'services-bento': return { title: 'Servizi', items: [] };
    case 'b2b-solutions': return { title: 'Per le Aziende', subtitle: 'Consulenza e reti', features: [] };
    default: return {};
  }
}
...
import { ServicesBentoSchema } from '@/components/services-bento';
import { B2BSchema } from '@/components/b2b-solutions';
import { HistorySchema } from '@/components/history-strip';
import { TestimonialsSchema } from '@/components/testimonials-grid';
import { ContactSchema } from '@/components/contact-section';

export const SECTION_SCHEMAS = {
  'header': HeaderSchema,
  'footer': FooterSchema,
  'hero': HeroSchema,
  'services-bento': ServicesBentoSchema,
  'b2b-solutions': B2BSchema,
  'history-strip': HistorySchema,
  'testimonials-grid': TestimonialsSchema,
  'contact-section': ContactSchema,
} as const;
...
import type { ServicesBentoData, ServicesBentoSettings } from '@/components/services-bento';
import type { B2BData, B2BSettings } from '@/components/b2b-solutions';
import type { HistoryData, HistorySettings } from '@/components/history-strip';
import type { TestimonialsData, TestimonialsSettings } from '@/components/testimonials-grid';
import type { ContactData, ContactSettings } from '@/components/contact-section';

export type SectionComponentPropsMap = {
  'header': { data: HeaderData; settings: HeaderSettings };
  'footer': { data: FooterData; settings: FooterSettings };
  'hero': { data: HeroData; settings: HeroSettings };
  'services-bento': { data: ServicesBentoData; settings: ServicesBentoSettings };
  'b2b-solutions': { data: B2BData; settings: B2BSettings };
  'history-strip': { data: HistoryData; settings: HistorySettings };
  'testimonials-grid': { data: TestimonialsData; settings: TestimonialsSettings };
  'contact-section': { data: ContactData; settings: ContactSettings };
};
...
declare module '@olonjs/core' {
  export interface SectionDataRegistry {
    ...
    'footer': FooterData;
    'hero': HeroData;
    'services-bento': ServicesBentoData;
    'b2b-solutions': B2BData;
    'history-strip': HistoryData;
    'testimonials-grid': TestimonialsData;
    'contact-section': ContactData;
  }
  export interface SectionSettingsRegistry {
    'header': HeaderSettings;
    'footer': FooterSettings;
    'hero': HeroSettings;
    'services-bento': ServicesBentoSettings;
    'b2b-solutions': B2BSettings;
    'history-strip': HistorySettings;
    'testimonials-grid': TestimonialsSettings;
    'contact-section': ContactSettings;
  }
}
