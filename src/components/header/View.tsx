...
import { isInAppPathHref } from '@/lib/isInAppPathHref'; // SPA navigation — see docs/decisions/ADR-002
    ...
    >
      <div className="max-w-[1200px] mx-auto px-8">
        ...
        <div className="flex h-20 items-center justify-between gap-6">
          <Link to="/" className="inline-flex flex-col items-stretch gap-0.5">
            <BitcomLogo
            ...
            <NavigationMenu>
              <NavigationMenuList className="gap-1">
                {navItems.map((item, idx) => {
                  const navClass = `rounded-[var(--local-radius-md)] px-4 py-2 text-sm font-medium transition hover:bg-[var(--local-surface)] ${
                    item.isCta
                ...
                <nav className="flex-1 overflow-y-auto px-6 py-8">
                  ...
                  <ul className="flex flex-col">
                    {navItems
                      ...
                      .map((item, idx) => {
                        const mobileClass = "group flex items-center justify-between gap-3 py-4 border-b border-[var(--border)] text-[1.25rem] font-display font-semibold text-[var(--foreground)] hover:text-[var(--primary)] transition-colors";
                        const inner = (
                          <>
                        ...
                        return (
                          <li key={item.href + '-mobile-' + idx}>
                            <SheetClose asChild>
                              {isInAppPathHref(item.href) ? (
                                <Link to={item.href} className={mobileClass}>
                                  {inner}
                                </Link>
                              ) : (
                  ...
                  {navItems.some((item) => item.isCta) && (
                    <div className="mt-8 flex flex-col gap-3">
                      {navItems
                        .filter((item) => item.isCta)
                        .map((item, idx) => {
                          const ctaClass = "inline-flex items-center justify-center gap-2 rounded-[var(--theme-radius-md)] bg-[var(--primary)] text-[var(--primary-foreground)] px-5 py-3.5 text-base font-semibold hover:opacity-90 transition-opacity";
                          const inner = (
                            <>
                              {item.label}
                          ...
                          return (
                            <SheetClose asChild key={item.href + '-cta-' + idx}>
                              {isInAppPathHref(item.href) ? (
                                <Link to={item.href} className={ctaClass}>
                                  {inner}
                                </Link>
                              ) : (
