...
END_OF_FILE_CONTENT
mkdir -p "src/components/bento-features"
echo "Creating src/components/bento-features/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/bento-features/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/bento-features/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/bento-features/index.ts"
export { BentoFeatures as default, BentoFeatures } from './View';
export { BentoFeaturesSchema } from './schema';
export type { BentoFeaturesData, BentoFeaturesSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/bento-features/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/bento-features/schema.ts"
import { z } from 'zod';
...
END_OF_FILE_CONTENT
echo "Creating src/components/bento-features/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/bento-features/types.ts"
import { z } from 'zod';
...
export { CtaBanner as default, CtaBanner } from './View';
...
export const Footer: React.FC<{ data: FooterData; settings: FooterSettings }> = ({ data }) => {
  ...
  return (
    <footer
      ...
      className="relative z-0 border-t border-[var(--local-border)] bg-[var(--local-bg)] py-16 lg:py-24"
    >
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-4">
          <div className="lg:col-span-2">
            <h3 className="font-display text-2xl font-bold text-[var(--local-text)]" data-jp-field="brandText" style={{ letterSpacing: '-0.03em' }}>
              {data.brandText}
            </h3>
            {data.description && (
              <p className="mt-4 max-w-sm text-sm text-[var(--local-text-muted)] leading-relaxed" data-jp-field="description">
                {data.description}
              </p>
            )}
          </div>
          <div>
            <h4 className="font-display mb-6 font-semibold text-[var(--local-text)]">Contatti</h4>
            <div className="space-y-3 text-sm text-[var(--local-text-muted)]">
              <p data-jp-field="address">{data.address}</p>
              <p data-jp-field="phone">{data.phone}</p>
              <p data-jp-field="vat">{data.vat}</p>
            </div>
          </div>
          <div>
            <h4 className="font-display mb-6 font-semibold text-[var(--local-text)]">Link</h4>
            <ul className="space-y-3">
              {navItems.map((item, idx) => (
                <li key={idx}>
                  <a href={item.href} className="text-sm text-[var(--local-text-muted)] transition-colors hover:text-[var(--local-primary)]">
                    {item.label}
                  </a>
                </li>
              ))}
        ...
        <div className="flex flex-col items-center justify-between gap-4 text-xs text-[var(--local-text-muted)] sm:flex-row">
          <p data-jp-field="copyright">{data.copyright}</p>
          <p>Realizzato con OlonJS</p>
        </div>
      </div>
    </footer>
  );
...
export { Footer as default, Footer } from './View';
...
export const Header: React.FC<{ data: HeaderData; settings: HeaderSettings }> = ({ data }) => {
  ...
  React.useEffect(() => {
    ...
    if (!current) root.setAttribute('data-theme', prefersDark ? 'dark' : 'light');
  }, []);
  ...
  return (
    <header
      style={{
        '--local-bg': 'color-mix(in oklch, var(--background) 95%, transparent)',
        '--local-text': 'var(--foreground)',
        '--local-border': 'var(--border)',
      ...
      className="sticky top-0 z-50 border-b border-[var(--local-border)] bg-[var(--local-bg)]/95 backdrop-blur-xl"
    >
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
        {data.announcement && (
          <div className="border-b border-[var(--local-border)] py-2 text-center text-xs font-mono tracking-widest text-[var(--local-text)]/70 uppercase" data-jp-field="announcement">
            {data.announcement}
          </div>
        )}
        <div className="flex h-20 items-center justify-between gap-6">
          <a href="/" className="flex items-center gap-3">
            <MonitorSmartphone className="h-6 w-6 text-[var(--local-primary)]" />
            <div className="flex items-baseline gap-1">
              <span className="font-display text-2xl font-bold tracking-tight text-[var(--local-text)]" data-jp-field="logoText" style={{ letterSpacing: '-0.03em' }}>
                {data.logoText}
              </span>
              {data.logoHighlight && (
                <span className="font-mono text-xs uppercase tracking-widest text-[var(--local-primary)]" data-jp-field="logoHighlight">
                  {data.logoHighlight}
                </span>
              )}
            </div>
              ...
              <NavigationMenuList className="gap-2">
                {navItems.map((item, idx) => (
                  <NavigationMenuItem key={item.href + '-' + idx}>
                    <NavigationMenuLink
                      href={item.href}
                      className={`rounded-[var(--local-radius-md)] px-4 py-2 text-sm font-medium transition-colors ${
                        item.isCta 
                          ? 'bg-[var(--local-primary)] text-[var(--theme-colors-primary-foreground)] hover:opacity-90' 
                          : 'text-[var(--local-text)] hover:bg-[var(--local-surface)]'
                      }`}
                    >
                      {item.label}
                    </NavigationMenuLink>
          ...
          <div className="flex items-center gap-3 lg:hidden">
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="outline" size="icon" className="rounded-[var(--local-radius-md)] border-[var(--local-border)] bg-[var(--local-surface)] text-[var(--local-text)]">
                ...
                <div className="mt-8 flex flex-col gap-3">
                  {navItems.map((item, idx) => (
                    <a
                      ...
                      href={item.href}
                      className={`rounded-[var(--local-radius-md)] px-4 py-3 text-sm font-medium ${
                        item.isCta
                          ? 'bg-[var(--local-primary)] text-[var(--theme-colors-primary-foreground)]'
                          : 'border border-[var(--local-border)] text-[var(--local-text)]'
                      }`}
                    >
                      {item.label}
                    </a>
...
export { Header as default, Header } from './View';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/hero-split"
echo "Creating src/components/hero-split/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/hero-split/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/hero-split/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/hero-split/index.ts"
export { HeroSplit as default, HeroSplit } from './View';
export { HeroSplitSchema } from './schema';
export type { HeroSplitData, HeroSplitSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/hero-split/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/hero-split/schema.ts"
import { z } from 'zod';
...
END_OF_FILE_CONTENT
echo "Creating src/components/hero-split/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/hero-split/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/history-timeline"
echo "Creating src/components/history-timeline/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-timeline/View.tsx"
// Layout: Features=C (TIMELINE)
import React from 'react';
import type { HistoryTimelineData, HistoryTimelineSettings } from './types';

export const HistoryTimeline: React.FC<{ data: HistoryTimelineData; settings: HistoryTimelineSettings }> = ({ data }) => {
  return (
    <section
      style={{
        '--local-bg': 'var(--background)',
    ...
    >
      <div className="max-w-[800px] mx-auto px-6 lg:px-8">
        ...
        <div className="relative border-l-2 border-[var(--local-border)] ml-4 md:ml-0 md:border-none">
          {/* Central line for desktop */}
          <div className="hidden md:block absolute top-0 bottom-0 left-1/2 w-0.5 bg-[var(--local-border)] -translate-x-1/2" />
          ...
          <div className="space-y-16">
            {data.items.map((item, idx) => {
              ...
              return (
                <div 
                  key={item.id || `time-${idx}`}
                  className={`relative flex flex-col md:flex-row items-start md:items-center ${isEven ? 'md:flex-row-reverse' : ''}`}
                  data-jp-item-id={item.id || `time-${idx}`}
                  data-jp-item-field="items"
                >
                  {/* Timeline dot */}
                  <div className="absolute left-[-9px] md:left-1/2 md:-translate-x-1/2 w-4 h-4 rounded-full bg-[var(--local-primary)] ring-4 ring-[var(--local-bg)] z-10" />
                  
                  <div className={`pl-8 md:pl-0 w-full md:w-1/2 ${isEven ? 'md:pr-16 md:text-right' : 'md:pl-16 md:text-left'}`}>
                    <div className="font-mono text-xl font-bold text-[var(--local-primary)] mb-2">
                      {item.year}
                    </div>
                    <h3 className="font-display text-2xl font-bold text-[var(--local-text)] mb-3">
                      {item.title}
                    </h3>
...
END_OF_FILE_CONTENT
echo "Creating src/components/history-timeline/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-timeline/index.ts"
export { HistoryTimeline as default, HistoryTimeline } from './View';
export { HistoryTimelineSchema } from './schema';
export type { HistoryTimelineData, HistoryTimelineSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/history-timeline/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-timeline/schema.ts"
import { z } from 'zod';
import { BaseSectionData, BaseArrayItem } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/history-timeline/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/history-timeline/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/process-accordion"
echo "Creating src/components/process-accordion/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/process-accordion/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/process-accordion/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/process-accordion/index.ts"
export { ProcessAccordion as default, ProcessAccordion } from './View';
export { ProcessAccordionSchema } from './schema';
export type { ProcessAccordionData, ProcessAccordionSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/process-accordion/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/process-accordion/schema.ts"
import { z } from 'zod';
import { BaseSectionData, BaseArrayItem } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/process-accordion/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/process-accordion/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
END_OF_FILE_CONTENT
mkdir -p "src/components/reviews-carousel"
echo "Creating src/components/reviews-carousel/View.tsx..."
cat << 'END_OF_FILE_CONTENT' > "src/components/reviews-carousel/View.tsx"
...
END_OF_FILE_CONTENT
echo "Creating src/components/reviews-carousel/index.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/reviews-carousel/index.ts"
export { ReviewsCarousel as default, ReviewsCarousel } from './View';
export { ReviewsCarouselSchema } from './schema';
export type { ReviewsCarouselData, ReviewsCarouselSettings } from './types';

END_OF_FILE_CONTENT
echo "Creating src/components/reviews-carousel/schema.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/reviews-carousel/schema.ts"
import { z } from 'zod';
import { BaseSectionData, BaseArrayItem } from '@olonjs/core';
...
END_OF_FILE_CONTENT
echo "Creating src/components/reviews-carousel/types.ts..."
cat << 'END_OF_FILE_CONTENT' > "src/components/reviews-carousel/types.ts"
import { z } from 'zod';
import { BaseSectionSettingsSchema } from '@olonjs/core';
...
export { StatsBand as default, StatsBand } from './View';
...
cat << 'END_OF_FILE_CONTENT' > "src/data/config/menu.json"
{
  ...
  "footer": [
    { "label": "Home", "href": "/" },
    { "label": "Privati e Riparazioni", "href": "/privati" },
...
cat << 'END_OF_FILE_CONTENT' > "src/data/config/site.json"
{
  "header": {
    ...
    "data": {
      ...
      "announcement": "Via Simone Cuccia 1B, Palermo | 091 306740",
  ...
  "footer": {
    ...
    "data": {
      "brandText": "BIT COM",
      "description": "B.S. Informatica S.n.c. dal 1996 al servizio di Palermo. Vendita, assistenza e soluzioni IT integrate per privati, aziende e professionisti.",
      "address": "Via Simone Cuccia 1B, 90144 Palermo",
      "phone": "091 306740",
      "vat": "P. IVA 06244880826",
      "copyright": "© 2024 B.S. Informatica S.n.c. Tutti i diritti riservati.",
      "menu": { "$ref": "../config/menu.json#/footer" }
    },
    "settings": {}
  },
  "identity": { "title": "BIT COM - B.S. Informatica" },
  "pages": []
}
...
echo "Creating src/data/pages/aziende.json..."
cat << 'END_OF_FILE_CONTENT' > "src/data/pages/aziende.json"
{
  "id": "aziende-page",
  "slug": "aziende",
  "meta": {
    "title": "Soluzioni per Aziende | BIT COM Palermo",
    "description": "Progettazione reti, consulenza IT e assistenza on-site per le aziende di Palermo e provincia."
  },
  "sections": [
    {
      "id": "az-hero",
      ...
      "data": {
        "label": "Divisione B2B",
        "title": "L'infrastruttura IT della tua azienda, in mani sicure.",
        "description": "Non siamo solo un negozio. Dal 2009 supportiamo imprese, studi professionali e attività commerciali con reti stabili, software su misura e assistenza continuativa.",
        "primaryCta": { "id": "az-cta1", "label": "Richiedi un sopralluogo", "href": "tel:091306740", "variant": "primary" },
        "image": { "url": "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?q=80&w=2034&auto=format&fit=crop", "alt": "Server rack aziendale e cavi di rete" }
      },
      "settings": {}
    },
    {
      ...
      "data": {
        ...
        "items": [
          { "id": "bf1", "span": "2", "title": "Progettazione Reti Aziendali", "description": "Installazione e configurazione di reti locali, Wi-Fi aziendale sicuro, firewall e sistemi di backup. Gestiamo architetture complesse per garantire operatività senza interruzioni." },
          { "id": "bf2", "span": "1", "title": "Sviluppo Software", "description": "Realizzazione di applicativi e software gestionali personalizzati sulle specifiche esigenze della tua organizzazione." },
          { "id": "bf3", "span": "1", "title": "Fornitura Hardware", "description": "Dalle postazioni operative (PC, monitor) alle stampanti di rete e server, forniamo tutto l'equipaggiamento necessario." },
          { "id": "bf4", "span": "2", "title": "Consulenza e Formazione", "description": "Ti aiutiamo a ottimizzare i flussi di lavoro attraverso la tecnologia. Offriamo formazione su informatica, automazione e organizzazione aziendale per il tuo team." }
        ]
      },
      "settings": {}
    },
    {
      "id": "az-process",
      "type": "process-accordion",
      "data": {
        "label": "METODO",
        "title": "Come lavoriamo",
        "description": "Un approccio rigoroso per garantire che la tua azienda non si fermi mai.",
        "items": [
          { "id": "p1", "title": "Analisi e Sopralluogo", "description": "Tutto inizia con un contatto e una comprensione reale delle tue necessità. Se necessario, eseguiamo un sopralluogo tecnico gratuito presso la tua sede a Palermo o provincia." },
          { "id": "p2", "title": "Progetto e Preventivo", "description": "Sviluppiamo una soluzione tecnica su misura e forniamo un preventivo dettagliato e trasparente. Nessuna sorpresa, costi certi." },
          { "id": "p3", "title": "Implementazione", "description": "Installiamo le reti, configuriamo i software, prepariamo le postazioni e formiamo il personale per rendere tutto operativo nel minor tempo possibile." },
          { "id": "p4", "title": "Assistenza Continuativa", "description": "Il nostro lavoro non finisce con l'installazione. Forniamo assistenza remota e on-site per mantenere i sistemi aggiornati, sicuri e performanti nel tempo." }
        ]
      },
      "settings": {}
    },
    {
      ...
      "type": "cta-banner",
      "data": {
        "title": "Pronto a ottimizzare l'IT della tua azienda?",
        "description": "Affidati a chi ha trent'anni di esperienza sul campo. Interventi rapidi su tutta Palermo e provincia.",
        "primaryCta": { "id": "cta-az", "label": "Contattaci ora", "href": "/contatti", "variant": "primary" }
      },
      "settings": {}
    }
  ]
...
END_OF_FILE_CONTENT
echo "Creating src/data/pages/chi-siamo.json..."
cat << 'END_OF_FILE_CONTENT' > "src/data/pages/chi-siamo.json"
{
  "id": "chi-siamo-page",
  "slug": "chi-siamo",
  "meta": {
    "title": "La Nostra Storia | BIT COM Palermo",
    "description": "Scopri la storia di BIT COM, dal 1996 a Palermo. Marco Borgese e Gabriele Santoro al servizio della tua tecnologia."
  },
  "sections": [
    {
      "id": "about-hero",
      ...
      "data": {
        "label": "Chi Siamo",
        "title": "Una storia di passione e circuiti stampati.",
        "description": "Siamo nati nel 1996, quando avere un computer in casa era un'eccezione e internet un suono metallico dal modem. Da allora siamo nello stesso settore, nella stessa città.",
        "image": { "url": "https://images.unsplash.com/photo-1581092160562-40aa08e78837?q=80&w=2070&auto=format&fit=crop", "alt": "Tecnico informatico al lavoro su una scheda madre" }
      },
      "settings": {}
    },
    {
      ...
      "data": {
        "label": "MILESTONES",
        "title": "Quasi 30 anni di evoluzione",
        "items": [
          { "id": "t1", "year": "1996", "title": "Le Origini", "description": "Iniziamo l'attività in un'epoca pionieristica per l'informatica di consumo. La missione è semplice: portare la tecnologia nelle case e farla funzionare." },
          { "id": "t2", "year": "2009", "title": "Nasce B.S. Informatica", "description": "Costituzione ufficiale della società B.S. Informatica di Borgese Marco & C. S.n.c., strutturando l'attività in Via Simone Cuccia, in zona Libertà." },
          { "id": "t3", "year": "Oggi", "title": "Oltre il Negozio", "description": "Guidata dai titolari Marco Borgese e Gabriele Santoro, l'attività si è evoluta. Non solo un negozio di quartiere per le riparazioni, ma una realtà capace di gestire reti aziendali complesse, sviluppo software e consulenza B2B per tutta la provincia." }
        ]
      },
      "settings": {}
    },
    {
      ...
      "data": {
        ...
        "title": "Cosa ci guida ogni giorno",
        "items": [
          { "id": "v1", "title": "Competenza Tecnica", "description": "Un bagaglio costruito in decenni di mestiere. Conosciamo l'hardware vecchio e le architetture nuove." },
          { "id": "v2", "title": "Cortesia ed Empatia", "description": "In negozio si entra con un problema o un'urgenza lavorativa, si deve uscire con una soluzione chiara e un sorriso." },
          { "id": "v3", "title": "Puntualità", "description": "Sappiamo quanto un dispositivo fermo incida sulla vita o sul lavoro. Diamo tempi realistici e li rispettiamo." },
          { "id": "v4", "title": "Trasparenza", "description": "Preventivo sempre fornito prima di iniziare qualsiasi tipo di intervento. Niente sorprese sul conto finale." },
          { "id": "v5", "title": "Onestà Assoluta", "description": "Se una riparazione non conviene economicamente rispetto all'acquisto del nuovo, te lo diciamo senza esitazioni." }
        ]
      },
      "settings": {}
    }
...
cat << 'END_OF_FILE_CONTENT' > "src/data/pages/contatti.json"
{
  ...
  "slug": "contatti",
  "meta": {
    "title": "Contatti e Orari | BIT COM Palermo",
    "description": "Vieni a trovarci in Via Simone Cuccia 1B a Palermo. Scopri gli orari e i contatti per assistenza informatica."
  },
  "sections": [
    {
      "id": "contact-hero",
      ...
      "data": {
        "label": "Contattaci",
        "title": "Siamo qui per risolvere.",
        "description": "Negozio storico di quartiere in zona Libertà. Passa a trovarci per un preventivo gratuito o chiamaci per organizzare un intervento in azienda.",
        "primaryCta": { "id": "c1", "label": "Chiama: 091 306740", "href": "tel:091306740", "variant": "primary" },
        "image": { "url": "https://images.unsplash.com/photo-1573164713988-8665fc963095?q=80&w=2069&auto=format&fit=crop", "alt": "Ingresso del negozio BIT COM" }
      },
      "settings": {}
    },
    {
      ...
      "data": {
        "label": "INFORMAZIONI",
        "title": "Dove siamo e Orari",
        "items": [
          { "id": "ci1", "span": "2", "title": "Il Negozio", "description": "Siamo a Palermo in Via Simone Cuccia 1B (CAP 90144), traversa di Viale Libertà. \n\nSocietà: B.S. Informatica di Borgese Marco & C. S.n.c.\nSede legale: Via Luigi Pirandello 36, Palermo\nP.IVA: 06244880826" },
          { "id": "ci2", "span": "1", "title": "Orari di Apertura", "description": "Lunedì – Venerdì:\n09:00 – 13:00\n16:00 – 19:30\n\nSabato:\n10:00 – 13:00\n\nDomenica: Chiuso" }
        ]
      },
      "settings": {}
    }
...
cat << 'END_OF_FILE_CONTENT' > "src/data/pages/home.json"
{
  ...
  "slug": "home",
  "meta": {
    "title": "BIT COM - Negozio Informatica Palermo",
    "description": "Vendita, assistenza e riparazioni computer e smartphone a Palermo. Soluzioni IT per aziende."
  },
  "sections": [
    {
      "id": "home-hero",
      "type": "hero-split",
      "data": {
        "label": "A Palermo dal 1996",
        "title": "Tecnologia che funziona, persone che ascoltano.",
        "description": "Da quasi trent'anni siamo il punto di riferimento in zona Libertà per l'assistenza tecnica, la vendita e l'infrastruttura IT. Per i privati e per le imprese.",
        "primaryCta": { "id": "h-cta1", "label": "Scopri i servizi", "href": "#servizi", "variant": "primary" },
        "secondaryCta": { "id": "h-cta2", "label": "Per le Aziende", "href": "/aziende", "variant": "secondary" },
        "image": { "url": "https://images.unsplash.com/photo-1597872253359-f191b72e90e9?q=80&w=2070&auto=format&fit=crop", "alt": "Interno di un moderno centro assistenza tecnica con computer in riparazione" }
      },
      "settings": {}
    },
    {
      ...
      "data": {
        "items": [
          { "id": "s1", "value": "1996", "label": "Anno di fondazione" },
          { "id": "s2", "value": "28+", "label": "Anni di esperienza" },
          { "id": "s3", "value": "B2C", "label": "Riparazioni e Vendita" },
          { "id": "s4", "value": "B2B", "label": "Reti e Consulenza" }
        ]
      },
      "settings": {}
    },
    {
      ...
      "data": {
        ...
        "title": "Cosa facciamo in negozio",
        "items": [
          { "id": "srv1", "tag": "VENDITA", "title": "PC e Notebook", "description": "Vendita di computer fissi e portatili, sia nuovi che rigenerati garantiti. Per studio, ufficio o gaming." },
          { "id": "srv2", "tag": "RIPARAZIONE", "title": "Assistenza Smartphone", "description": "Riparazione professionale di smartphone e tablet, specializzati su dispositivi Apple e Android (Samsung, Huawei)." },
          { "id": "srv3", "tag": "ACCESSORI", "title": "Cartucce e Toner", "description": "Fornitura di consumabili, cartucce e toner compatibili per tutte le principali marche (Brother, Canon, Epson)." }
        ]
      },
      "settings": {}
    },
    {
      ...
      "data": {
        ...
        "items": [
          { "id": "r1", "author": "Vito Luca", "quote": "Qualità e servizio eccellenti! Sicuramente consigliato." },
          { "id": "r2", "author": "Domenico Scammacca", "quote": "Molto preparati e professionali, cortesia e soluzioni ottimali li distinguono." },
          { "id": "r3", "author": "Antonino Matranga", "quote": "Professionalità e puntualità." },
          { "id": "r4", "author": "Gabriele Citarrella", "quote": "Molto professionali, ottime apparecchiature." }
        ]
      },
      "settings": {}
    },
    {
      ...
      "type": "cta-banner",
      "data": {
        "title": "Il tuo computer ha un problema?",
        "description": "Porta il tuo dispositivo in Via Simone Cuccia 1B. Diagnosi attenta, preventivo chiaro prima di intervenire, massima onestà. Se non conviene ripararlo, te lo diciamo.",
        "primaryCta": { "id": "cta-home", "label": "Chiama per informazioni", "href": "tel:091306740", "variant": "primary" }
      },
      "settings": {}
    }
  ]
...
:root {
  /* Default Dark Mode mapped to design system base */
...
:root[data-theme="light"] {
  /* Light Mode mapped to design system mode overrides */
...
export const ComponentRegistry: {
  ...
  'process-accordion': ProcessAccordion,
...
const sectionTypeLabels: Record<string, string> = {
  ...
  'bento-features': 'Bento Features',
  'service-cards': 'Service Cards',
  'process-accordion': 'Process Accordion',
...
function getDefaultSectionData(type: string): Record<string, unknown> {
  switch (type) {
    case 'hero-split': return { title: 'Nuovo Titolo' };
    case 'bento-features': return { title: 'Caratteristiche', items: [] };
    case 'service-cards': return { title: 'Servizi', items: [] };
    case 'process-accordion': return { title: 'Come Lavoriamo', items: [] };
    case 'history-timeline': return { title: 'La Nostra Storia', items: [] };
    case 'reviews-carousel': return { title: 'Recensioni', items: [] };
    case 'cta-banner': return { title: 'Contattaci' };
    case 'stats-band': return { items: [] };
    default: return {};
  }
}
...
import { BentoFeaturesSchema } from '@/components/bento-features';
import { ServiceCardsSchema } from '@/components/service-cards';
import { ProcessAccordionSchema } from '@/components/process-accordion';
import { HistoryTimelineSchema } from '@/components/history-timeline';
import { ReviewsCarouselSchema } from '@/components/reviews-carousel';
...
export const SECTION_SCHEMAS = {
  ...
  'bento-features': BentoFeaturesSchema,
  'service-cards': ServiceCardsSchema,
  'process-accordion': ProcessAccordionSchema,
  'history-timeline': HistoryTimelineSchema,
  'reviews-carousel': ReviewsCarouselSchema,
...
import type { BentoFeaturesData, BentoFeaturesSettings } from '@/components/bento-features';
import type { ServiceCardsData, ServiceCardsSettings } from '@/components/service-cards';
import type { ProcessAccordionData, ProcessAccordionSettings } from '@/components/process-accordion';
import type { HistoryTimelineData, HistoryTimelineSettings } from '@/components/history-timeline';
import type { ReviewsCarouselData, ReviewsCarouselSettings } from '@/components/reviews-carousel';
...
export type SectionComponentPropsMap = {
  ...
  'bento-features': { data: BentoFeaturesData; settings: BentoFeaturesSettings };
  'service-cards': { data: ServiceCardsData; settings: ServiceCardsSettings };
  'process-accordion': { data: ProcessAccordionData; settings: ProcessAccordionSettings };
  'history-timeline': { data: HistoryTimelineData; settings: HistoryTimelineSettings };
  'reviews-carousel': { data: ReviewsCarouselData; settings: ReviewsCarouselSettings };
...
declare module '@olonjs/core' {
  export interface SectionDataRegistry {
    ...
    'bento-features': BentoFeaturesData;
    'service-cards': ServiceCardsData;
    'process-accordion': ProcessAccordionData;
    'history-timeline': HistoryTimelineData;
    'reviews-carousel': ReviewsCarouselData;
  ...
  export interface SectionSettingsRegistry {
    ...
    'bento-features': BentoFeaturesSettings;
    'service-cards': ServiceCardsSettings;
    'process-accordion': ProcessAccordionSettings;
    'history-timeline': HistoryTimelineSettings;
    'reviews-carousel': ReviewsCarouselSettings;
